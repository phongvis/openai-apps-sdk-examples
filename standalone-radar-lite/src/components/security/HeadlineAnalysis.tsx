...
const descriptorColor: Record<string, string> = {
...
const formatIndustryName = (industryRaw: string): string => {
  return industryRaw
    .split(' ')
    .filter(Boolean)
...
const HeadlineAnalysis: React.FC<HeadlineAnalysisProps> = ({
  scores,
  inputLabel,
  industry,
}) => {
  ...
  const baselineOverall = baseline.find(
    (score) => score.category === 'overall'
  );
